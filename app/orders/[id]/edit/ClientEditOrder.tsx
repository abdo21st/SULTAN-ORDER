...
export default function ClientEditOrder({ id }: { id: string }) {
        ...
        if (id) {
            ...
            if (order) {
                if (order.status !== 'DRAFT') {
                    alert('عذراً، لا يمكن تعديل الطلب بعد اعتماده');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        try {
            orderService.updateDetails(id, {
                customerName: formData.customerName,
                customerPhone: formData.customerPhone,
                description: formData.description,
                dueDate: formData.dueDate,
                // @ts-ignore - Store needs update to accept amounts in updateDetails
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <div className="mb-6 flex items-center gap-2 text-gray-500">
                <Link href={`/orders/${id}`} className="hover:text-gray-900 transition-colors">
                    <ArrowRight size={20} />
                </Link>
            ...
            <div className="bg-white rounded-xl shadow-lg border border-orange-100 p-8">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-2xl font-bold text-gray-900">تعديل بيانات الطلب</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            <label htmlFor="paidAmount" className="block text-sm font-medium text-gray-700 mb-2">المبلغ المدفوع</label>
                            <input
                                id="paidAmount"
                                required
                                type="number"
                        ...
                        <div>
                            <label htmlFor="remainingAmount" className="block text-sm font-medium text-gray-700 mb-2">المتبقي</label>
                            <input
                                id="remainingAmount"
                                type="number"
                                disabled
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">صورة الطلب</label>
                        <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 flex flex-col items-center justify-center text-gray-500 bg-gray-50 hover:bg-gray-100 transition-colors relative">
                            {imagePreview ? (
                                ...
                                <>
                                    ...
                                    <span className="text-sm mb-2">اضغط لتغيير الصورة</span>
                                    <input
                                        type="file"
                                        accept="image/*"
                    ...
                    <div className="flex gap-4">
                        <Link href={`/orders/${id}`} className="w-1/3 bg-gray-100 text-gray-700 font-bold py-3 rounded-lg hover:bg-gray-200 text-center transition-colors">
                            إلغاء
                        </Link>
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-2/3 bg-orange-600 text-white font-bold py-3 rounded-lg hover:bg-orange-700 active:scale-95 transition-all shadow-lg shadow-orange-200"
                        >
